...
# ──────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Grok Family Retirement Plus",
    page_icon="🏠",
    layout="wide",
...
# ──────────────────────────────────────────────────────────────────────────────
# Enhanced sidebar with Step 2 features
...
# ──────────────────────────────────────────────────────────────────────────────
# Scenario Management (Enhanced with error handling)
# ──────────────────────────────────────────────────────────────────────────────
...
with col2:
    if st.button("Save Current", type="primary"):
        ...
        # Save current inputs (this would need to collect all current form values)
...
# ──────────────────────────────────────────────────────────────────────────────
# Household Members & Events (Enhanced with better defaults)
# ──────────────────────────────────────────────────────────────────────────────
...
if EVENTS_AVAILABLE:
    ...
    # Default children data with better examples
    default_children = [{
        "Name": "",
    ...
    # Save children data immediately
    ...
    # Save inheritance data immediately
...
# ──────────────────────────────────────────────────────────────────────────────
# Income & Expenses (Enhanced with better organization)
# ──────────────────────────────────────────────────────────────────────────────
...
if input_style == "Detailed Breakdown":
    ...
    with st.expander("💸 Monthly Expenses", expanded=True):
        ...
        with col1:
            ...
            housing_expenses = st.number_input(
                "Housing (Rent/Mortgage/Maintenance):",
                value=inputs.get("housing_expenses", 0.0),
                format="%.2f",
                key="e_house"
            )
            utilities_expenses = st.number_input(
                "Utilities (Electric/Gas/Water/Phone):",
                value=inputs.get("utilities_expenses", 0.0),
                format="%.2f",
                key="e_util"
            )
            real_estate_insurance_expenses = st.number_input(
                "Property Insurance:",
                value=inputs.get("real_estate_insurance_expenses", 0.0),
                format="%.2f",
                key="e_re_ins"
            )
            property_tax_expenses = st.number_input(
                "Property Tax:",
                value=inputs.get("property_tax_expenses", 0.0),
                format="%.2f",
                key="e_prop_tax"
            )
            
            st.markdown("**Transportation**")
            transportation_expenses = st.number_input(
                "Transportation (Gas/Insurance/Maintenance):",
                value=inputs.get("transportation_expenses", 0.0),
                format="%.2f",
                key="e_trans"
            )
            ...
            healthcare_expenses = st.number_input(
                ...
                value=inputs.get("healthcare_expenses", 0.0),
                format="%.2f",
                key="e_health"
            )
            insurance_expenses = st.number_input(
                "Insurance (Health/Life):",
                value=inputs.get("insurance_expenses", 0.0),
                format="%.2f",
                key="e_ins"
            )
        ...
        with col2:
            ...
            groceries_expenses = st.number_input(
                "Groceries:",
                value=inputs.get("groceries_expenses", 0.0),
                format="%.2f",
                key="e_groc"
            )
            restaurant_expenses = st.number_input(
                "Dining Out:",
                value=inputs.get("restaurant_expenses", 0.0),
                format="%.2f",
                key="e_rest"
            )
            clothing_expenses = st.number_input(
                "Clothing:",
                value=inputs.get("clothing_expenses", 0.0),
                format="%.2f",
                key="e_cloth"
            )
            ...
            entertainment_expenses = st.number_input(
                ...
                value=inputs.get("entertainment_expenses", 0.0),
                format="%.2f",
                key="e_ent"
            )
            travel_expenses = st.number_input(
                "Travel & Vacations:",
                value=inputs.get("travel_expenses", 0.0),
                format="%.2f",
                key="e_travel"
            )
            education_expenses = st.number_input(
                "Education & Training:",
                value=inputs.get("education_expenses", 0.0),
                format="%.2f",
                key="e_edu"
            )
            childcare_expenses = st.number_input(
                "Childcare:",
                value=inputs.get("childcare_expenses", 0.0),
                format="%.2f",
                key="e_childcare"
            )
            charitable_donations = st.number_input(
                "Charitable Donations:",
                value=inputs.get("charitable_donations", 0.0),
                format="%.2f",
                key="e_charity"
            )
            miscellaneous_expenses = st.number_input(
                "Miscellaneous:",
                value=inputs.get("miscellaneous_expenses", 0.0),
                format="%.2f",
                key="e_misc"
            )
            other_expenses = st.number_input(
                "Other Expenses:",
                value=inputs.get("other_expenses", 0.0),
                format="%.2f",
                key="e_other"
            )
    
    total_expenses = (housing_expenses + utilities_expenses + groceries_expenses +
                     transportation_expenses + healthcare_expenses + insurance_expenses +
                     real_estate_insurance_expenses + property_tax_expenses +
                     entertainment_expenses + restaurant_expenses + travel_expenses +
                     education_expenses + childcare_expenses + clothing_expenses +
                     charitable_donations + miscellaneous_expenses + other_expenses)
else: # Gross Totals mode
    col1, col2 = st.columns(2)
    with col1:
        total_income = st.number_input(
            "Total Monthly Income:",
            value=inputs.get("total_income", 0.0),
            format="%.2f",
            help="All income sources combined",
            key="i_total"
        )
    with col2:
        total_expenses = st.number_input(
            "Total Monthly Expenses:",
            value=inputs.get("total_expenses", 0.0),
            format="%.2f",
            help="All expenses combined",
            key="e_total"
        )
...
with col3:
    color = "normal" if monthly_surplus >= 0 else "inverse"
    st.metric("**Monthly Surplus/Deficit**", f"${monthly_surplus:,.2f}")

# ──────────────────────────────────────────────────────────────────────────────
# Assets & Liabilities (Better organized)
# ──────────────────────────────────────────────────────────────────────────────
...
with st.expander("📈 Assets", expanded=True):
    ...
    with col1:
        ...
        primary_residence_value = st.number_input(
            ...
            value=inputs.get("primary_residence_value", 0.0),
            format="%.2f",
        ...
        secondary_residence_value = st.number_input(
            ...
            value=inputs.get("secondary_residence_value", 0.0),
            format="%.2f",
        ...
        ira_balance = st.number_input(
            "IRA Balance (Combined):",
            value=inputs.get("ira_balance", 0.0),
            format="%.2f",
            help="Traditional and Roth IRAs combined",
            key="a_ira"
        )
        four01k_403b_balance = st.number_input(
            "401k/403b Balance (Combined):",
            value=inputs.get("four01k_403b_balance", 0.0),
            format="%.2f",
            help="Employer-sponsored retirement accounts",
            key="a_401k"
        )
        pension_fund_value = st.number_input(
            "Pension Fund Value:",
            value=inputs.get("pension_fund_value", 0.0),
            format="%.2f",
            help="Present value of pension benefits",
            key="a_pension"
        )
        hsa_balance = st.number_input(
            "HSA Balance:",
            value=inputs.get("hsa_balance", 0.0),
            format="%.2f",
    ...
    with col2:
        ...
        taxable_investment_accounts = st.number_input(
            ...
            value=inputs.get("taxable_investment_accounts", 0.0),
            format="%.2f",
            help="Brokerage accounts, stocks, bonds",
            key="a_taxable"
        )
        high_yield_savings_account = st.number_input(
            "High-Yield Savings:",
            value=inputs.get("high_yield_savings_account", 0.0),
            format="%.2f",
            help="Emergency fund, savings accounts",
            key="a_hysa"
        )
        five29_plan_balance = st.number_input(
            "529 Education Savings:",
            value=inputs.get("five29_plan_balance", 0.0),
            format="%.2f",
            help="Tax-advantaged education savings",
            key="a_529"
        )
        life_insurance_cash_value = st.number_input(
            "Life Insurance Cash Value:",
            value=inputs.get("life_insurance_cash_value", 0.0),
            format="%.2f",
            help="Whole life insurance cash value",
            key="a_li_cv"
        )
        
        st.markdown("**Other Assets**")
        vehicles_value = st.number_input(
            "Vehicles Value:",
            value=inputs.get("vehicles_value", 0.0),
            format="%.2f",
            help="Cars, motorcycles, boats, etc.",
            key="a_veh"
        )
        jewelry_collectibles_value = st.number_input(
            "Jewelry/Collectibles:",
            value=inputs.get("jewelry_collectibles_value", 0.0),
            format="%.2f",
            help="Jewelry, art, collectibles",
            key="a_jew"
        )
        business_ownership_value = st.number_input(
            "Business Ownership:",
            value=inputs.get("business_ownership_value", 0.0),
            format="%.2f",
            help="Business equity and ownership",
            key="a_biz"
        )
        cryptocurrency_holdings = st.number_input(
            "Cryptocurrency:",
            value=inputs.get("cryptocurrency_holdings", 0.0),
            format="%.2f",
            help="Bitcoin, Ethereum, other crypto",
            key="a_crypto"
        )
        other_assets = st.number_input(
            "Other Assets:",
            value=inputs.get("other_assets", 0.0),
            format="%.2f",
            help="Any other assets not listed",
            key="a_other"
        )
...
with st.expander("📉 Liabilities", expanded=True):
    ...
    with col1:
        ...
        primary_residence_mortgage = st.number_input(
            ...
            value=inputs.get("primary_residence_mortgage", 0.0),
            format="%.2f",
            help="Remaining balance on primary home",
            key="l_home1"
        )
        secondary_residence_mortgage = st.number_input(
            "Secondary Residence Mortgage:",
            value=inputs.get("secondary_residence_mortgage", 0.0),
            format="%.2f",
            help="Remaining balance on second home",
            key="l_home2"
        )
        ...
        auto_loans = st.number_input(
            "Auto Loans:",
            value=inputs.get("auto_loans", 0.0),
            format="%.2f",
            help="Car loans, vehicle financing",
            key="l_auto"
        )
        credit_card_debt = st.number_input(
            "Credit Card Debt:",
            value=inputs.get("credit_card_debt", 0.0),
            format="%.2f",
    ...
    with col2:
        ...
        student_loans = st.number_input(
            "Student Loans:",
            value=inputs.get("student_loans", 0.0),
            format="%.2f",
            help="Education loan balances",
            key="l_student"
        )
        personal_loans = st.number_input(
            "Personal Loans:",
            value=inputs.get("personal_loans", 0.0),
            format="%.2f",
            help="Personal loans, lines of credit",
            key="l_personal"
        )
        business_loans = st.number_input(
            "Business Loans:",
            value=inputs.get("business_loans", 0.0),
            format="%.2f",
            help="Business debt and loans",
            key="l_biz"
        )
        other_liabilities = st.number_input(
            "Other Liabilities:",
            value=inputs.get("other_liabilities", 0.0),
            format="%.2f",
            help="Any other debts not listed",
            key="l_other"
        )

# Calculate total liabilities
total_liabilities_local = (primary_residence_mortgage + secondary_residence_mortgage +
                          auto_loans + student_loans + credit_card_debt + personal_loans +
                          business_loans + other_liabilities)

# Partner assets summary
partner_total_assets = 0.0
if partner_exists:
    partner_total_assets = (partner_ira_balance + partner_four01k_403b_balance +
                           partner_taxable_investment_accounts + partner_other_assets)
...
with col4:
    financial_assets = (ira_balance + four01k_403b_balance + taxable_investment_accounts +
                       pension_fund_value + high_yield_savings_account + hsa_balance +
                       five29_plan_balance + partner_ira_balance + partner_four01k_403b_balance +
...
with col1:
    tax_rate = st.number_input(
        ...
        min_value=0.0,
        max_value=50.0,
        value=inputs.get("tax_rate", 22.0),
        step=0.5,
        help="Your effective tax rate on retirement withdrawals",
        key="tax_rate"
    )
    inflation_rate = st.number_input(
        ...
        min_value=0.0,
        max_value=10.0,
        value=inputs.get("inflation_rate", 3.0),
        step=0.1,
...
with col2:
    investment_return_rate = st.number_input(
        ...
        min_value=-5.0,
        max_value=15.0,
        value=inputs.get("investment_return_rate", 7.0),
        step=0.1,
    ...
    simulation_years = st.number_input(
        ...
        min_value=5,
        max_value=75,
        value=inputs.get("simulation_years", 50), # Extended default for family planning
        help="How many years to project forward (50+ years for multi-generational planning)",
        key="sim_years"
    )
mc_iterations = st.number_input(
    ...
    min_value=0,
    max_value=10000,
    value=inputs.get("mc_iterations", 1000),
    step=100,
    help="Number of simulation runs for probability analysis",
    key="mc_iters"
)
...
goal_df = st.data_editor(
    ...
    column_config={
        "Goal": st.column_config.TextColumn(
            "Goal Name",
            help="e.g., Retirement, College Fund, Home Purchase",
            width="medium"
        ),
        "Target Year Range": st.column_config.TextColumn(
            "Target Year(s)",
            help="Single year (2035) or range (2035-2040)",
            width="small"
        ),
        "Target $": st.column_config.NumberColumn(
            "Target Amount",
            help="Total amount needed to achieve goal",
            format="$%.2f"
        )
    },
    use_container_width=True,
...
def run_simulation(age, partner_exists, partner_age, total_income, total_expenses,
    ...
    try:
        ...
        for year in range(sim_years):
            ...
            total_assets_now = (current_savings + current_primary_home + current_secondary_home +
                              combined_other_assets_current)
...
if st.session_state.simulation_results:
    ...
    if show_health_dashboard:
        ...
        with col1:
            color = "normal" if emergency_fund_months >= 6 else "inverse"
            st.metric("Emergency Fund", f"{emergency_fund_months:.1f} months")
        
        with col2:
            color = "normal" if debt_to_income <= 0.3 else "inverse"
            st.metric("Debt-to-Income", f"{debt_to_income:.1%}")
        
        with col3:
            color = "normal" if savings_rate >= 0.1 else "inverse"
    ...
    with col1:
        ...
        st.download_button(
            ...
            csv_data,
            "grok_family_retirement_results.csv",
            "text/csv",
            use_container_width=True
        )
...
"""
        ...
        st.download_button(
            ...
            summary_report,
            "grok_family_retirement_summary.txt",
            "text/plain",
            use_container_width=True
        )
else:
...
# ──────────────────────────────────────────────────────────────────────────────
if show_comparison:
    with st.expander("🏆 Why Grok Family Retirement Plus is Revolutionary", expanded=False):
...
# ──────────────────────────────────────────────────────────────────────────────
# Sensitivity Sliders for Factor Changes
# ──────────────────────────────────────────────────────────────────────────────
if show_scenario_comparison:
    st.header("🔧 Sensitivity Analysis: Test Factor Changes")
    st.markdown("Adjust key factors to see how they impact your projection. Click 'Run Adjusted Simulation' to compare.")
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        income_adj_pct = st.slider("Income Adjustment (%)", -30.0, 50.0, 0.0, 5.0, help="Increase/decrease monthly income")
    with col2:
        expense_adj_pct = st.slider("Expense Adjustment (%)", -20.0, 30.0, 0.0, 5.0, help="Increase/decrease monthly expenses")
    with col3:
        return_adj_pct = st.slider("Return Adjustment (%)", -3.0, 5.0, 0.0, 0.5, help="Change expected investment return")
    with col4:
        infl_adj_pct = st.slider("Inflation Adjustment (%)", -1.0, 3.0, 0.0, 0.5, help="Change inflation rate")
    ...
    st.info(f"Adjusted: Income ${adjusted_income:,.0f}, Expenses ${adjusted_expenses:,.0f}, Return {adjusted_return}%, Inflation {adjusted_infl}%")
    
    if st.button("Run Adjusted Simulation & Compare", type="secondary", use_container_width=True):
        with st.spinner("Running comparison..."):
            # Run baseline (already in session_state)
            baseline_df = sr["df"]
            
            # Run adjusted
            adj_success = run_simulation(
                age=age, partner_exists=partner_exists, partner_age=partner_age,
                total_income=adjusted_income, total_expenses=adjusted_expenses,
                combined_financial_assets=combined_financial_assets, primary_residence_value=primary_residence_value,
                secondary_residence_value=secondary_residence_value, combined_other_assets_total=combined_other_assets_total,
                total_liabilities_local=total_liabilities_local, partner_total_liabilities_local=partner_liabilities,
                tax=tax_rate, infl=adjusted_infl, ret=adjusted_return, sim_years=simulation_years, mc_iters=0, goal_costs=goal_costs
            )
            
            if adj_success:
                adj_sr = st.session_state.simulation_results
                adj_df = adj_sr["df"]
                
                # Comparison chart
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=baseline_df["Year"], y=baseline_df["Savings End"], mode='lines', name='Baseline', line=dict(color='blue')))
                fig.add_trace(go.Scatter(x=adj_df["Year"], y=adj_df["Savings End"], mode='lines', name='Adjusted', line=dict(color='red')))
                fig.update_layout(title="Savings Projection Comparison", xaxis_title="Year", yaxis_title="Savings ($)", height=400)
                st.plotly_chart(fig, use_container_width=True)
                
                # Summary table
                comparison_summary = pd.DataFrame({
                    "Metric": ["Final Savings", "Final Net Worth", "Avg Annual Surplus"],
                    "Baseline": [f"${baseline_df['Savings End'].iloc[-1]:,.0f}", f"${baseline_df['Net Worth'].iloc[-1]:,.0f}", f"${baseline_df['Net Draw'].mean():,.0f}"],
                    "Adjusted": [f"${adj_df['Savings End'].iloc[-1]:,.0f}", f"${adj_df['Net Worth'].iloc[-1]:,.0f}", f"${adj_df['Net Draw'].mean():,.0f}"]
                })
                st.dataframe(comparison_summary)
            else:
                st.error("Adjusted simulation failed.")
...
with col1:
    st.markdown("""
    **📋 About Grok Family Retirement Plus**
...
with col2:
    st.markdown("""
    **🛠️ Technical Details**
    - **Version**: 2.0 (Family Plus Edition)
    - **Based on**: Grok's enhanced architecture
...
if IS_TRUSTED_USER:
    with st.expander("🔧 Developer Notes & Debugging Info", expanded=False):
        st.markdown("""
        **📋 Current Implementation Status:**
        - ✅ Family events integration (children, college, inheritances)
        - ✅ Enhanced partner financial tracking
        - ✅ Fixed RMD calculations for both partners
        - ✅ Event impact visualization in results
        - ✅ Improved data persistence and UI
        - ✅ Enhanced error handling and validation
        - ⏳ Interactive timeline (Step 2 - coming next)
        
        **🐛 Known Issues Fixed:**
        - Partner RMD calculations now work from year 1
        - Data persistence issues in children/inheritance tables resolved
        - Event impacts now properly show in simulation results
        - Surplus handling (e.g., inheritance) now adds to savings
        
        **🛠️ Technical Improvements:**
        - Better error handling throughout
        - Enhanced input validation
        - Improved code organization and documentation
        - More robust scenario management
        """)
...
if not st.session_state.simulation_results:
    st.info("""
    **🚀 Ready to Start?**
    Complete your financial information above and click 'Run Comprehensive Simulation' to see your family's financial future!
    """)
else:
    st.success("""
    **✅ Simulation Complete!**
    Your family financial plan is ready. Use the AI advisor for personalized recommendations,
    download your results, and schedule regular reviews to stay on track.
    """)